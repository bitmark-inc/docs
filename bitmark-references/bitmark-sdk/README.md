# Bitmark SDK

## [Overview](bitmark-sdk-document.md#overview)

## [Getting Started](bitmark-sdk-document.md#getting-started)

* [Installation](bitmark-sdk-document.md#installation)
* [Get your API token](bitmark-sdk-document.md#get-your-api-token)
* [Configuration](bitmark-sdk-document.md#configuration)


## [Account](account.md#account)

* [Create an account](account.md#create-an-account)
* [Get the account number](account.md#get-the-account-number)
* [Export an account](account.md#export-an-account)
  * [Seed](account.md#seed)
  * [Recovery Phrase](account.md#recovery-phrase)
* [Import an account](account.md#import-an-account)
  * [Recover from seed](account.md#recover-from-seed)
  * [Recover from phrase](account.md#recover-from-phrase)
* [Sign and Verify](account.md#sign-and-verify)
* [Validate an account number](account.md#validate-an-account-number)


## [Actions](action.md)

* [Register an asset](action.md#register-an-asset)
* [Issue bitmarks](action.md#issue-bitmarks)
* [Transfer a bitmark](action.md#transfer-a-bitmark)
  * [Direct transfer](action.md#direct-transfer)
  * [Countersigned transfer](action.md#countersigned-transfer)
    * [Propose a bitmark transfer offer](action.md#propose-a-bitmark-transfer-offer)
    * [Query offering bitmarks](action.md#query-offering-bitmarks)
    * [Accept the bitmark transfer offer](action.md#accept-the-bitmark-transfer-offer)
    * [Reject the transfer offer](action.md#reject-the-transfer-offer)
    * [Cancel the transfer offer](action.md#cancel-the-transfer-offer)


## [Query](query.md#query)

* [Asset](query.md#asset)
  * [Record data structure](query.md#record-data-structure)
  * [Query for a specific asset](query.md#query-for-a-specific-asset)
  * [Query for a set of assets](query.md#query-for-a-set-of-assets)
    * [assets registered by the specific registrant](query.md#assets-registered-by-the-specific-registrant)
* [Transaction (tx)](query.md#transaction-tx)
  * [Record data structure](query.md#record-data-structure-1)
  * [Query for a specific transaction](query.md#query-for-a-specific-transaction)
  * [Query for a set of transactions](query.md#query-for-a-set-of-transactions)
    * [The provenance of a bitmark](query.md#the-provenance-of-a-bitmark)
    * [The transaction history of an account](query.md#the-transaction-history-of-an-account)
* [Bitmark](query.md#bitmark)
  * [Record data structure](query.md#record-data-structure-2)
    * [offer](query.md#offer)
  * [Query for a specific bitmark](query.md#query-for-a-specific-bitmark)
  * [Query for a set of bitmarks](query.md#query-for-a-set-of-bitmarks)
    * [bitmarks issued by the specific issuer](query.md#bitmarks-issued-by-the-specific-issuer)
    * [bitmarks owned by the specific owner](query.md#bitmarks-owned-by-the-specific-owner)
    * [bitmarks offered from the specific sender](query.md#bitmarks-offered-from-the-specific-sender)
    * [bitmarks offered to the specific receiver](query.md#bitmarks-offered-to-the-specific-receiver)


## [Store Seed](store_seed.md#store-seed)
  
* [Android](store_seed.md#android)
* [iOS](store_seed.md#ios)


## [Web Socket](websocket.md#web-socket)
  
* [Event](websocket.md#event)
* [Java](websocket.md#java)
    * [Connect/Disconnect](websocket.md#connectdisconnect)
    * [Subscribe/Unsubscribe](websocket.md#subscribeunsubscribe)
* [Swift](websocket.md#swift)

