
# Explore the Protocol Docs

## [Blockchain Overview](blockchain-overview.md)

* [Definitions](blockchain-overview.md#definitions)
* [Transactions: Asset, Issue and Transfer Records](blockchain-overview.md#transactions-asset-issue-and-transfer-records)
    * [Asset Record](blockchain-overview.md#asset-record)
    * [Issue Record](blockchain-overview.md#issue-record)
    * [Transfer Record](blockchain-overview.md#transfer-record)
* [Transactions: Share](blockchain-overview.md#transactions-share)
  * [Share Balance Record](blockchain-overview.md#share-balance-record)
  * [Share Grant Record](blockchain-overview.md#share-grant-record)
  * [Share Swap Record](blockchain-overview.md#share-swap-record)
* [Transactions: Block ownership record](blockchain-overview.md#transactions-block-ownership-record)
  * [Block Foundation Record](blockchain-overview.md#block-foundation-record)
  * [Block Owner Transfer Record](blockchain-overview.md#block-owner-transfer-record)
* [Blockchain Structure](blockchain-overview.md#blockchain-structure)
  * [Block Header](blockchain-overview.md#block-header)
  * [Merkle Tree](blockchain-overview.md#merkle-tree)
  * [Block Hashing (Argon2, difficulty, proof of work \.\.\.)](blockchain-overview.md#block-hashing-argon2-difficulty-proof-of-work-)
  
## [Block Validation and Synchronization](block-verification-and-synchronization.md#block-verification-and-synchronization)

* [Block Structure](block-verification-and-synchronization.md#block-structure)
* [Block Validation](block-verification-and-synchronization.md#block-validation)
* [Block Synchronization](block-verification-and-synchronization.md#block-synchronization)
  * [Proof of Work](block-verification-and-synchronization.md#proof-of-work)
  * [Majority Votes](block-verification-and-synchronization.md#majority-votes)

## [Block Mining](block-mining.md)

* [Overview](block-mining.md#overview)
* [Block Diagram](block-mining.md#block-diagram)
* [Hashing](block-mining.md#hashing)
* [Difficulty](block-mining.md#difficulty)
* [Communication](block-mining.md#communication)
* [Verification and Broadcast](block-mining.md#verification-and-broadcast)
* [Reward](block-mining.md#reward)

##  [Payment Verification](payment-verification.md#payment-verification)

* [Overview](payment-verification.md#overview)
* [Reservoir Module](payment-verification.md#reservoir-module)
  * [Processing a New Transaction](payment-verification.md#processing-a-new-transaction)
  * [Generating a Payment Id](payment-verification.md#generating-a-payment-id)
  * [Changing the State of a Paid Transaction](payment-verification.md#changing-the-state-of-a-paid-transaction)
  * [Pending and Verified Structures](payment-verification.md#pending-and-verified-structures)
* [Payment Module](payment-verification.md#payment-module)
  * [Using RPC APIs](payment-verification.md#using-rpc-apis)
  * [Using Discovery Service](payment-verification.md#using-discovery-service)
  * [Using Bitcoin Peer-to-Peer Protocol (WIP)](payment-verification.md#using-bitcoin-peer-to-peer-protocol-wip)

## [Storage Databases](storage-databases.md)

* [Blocks DB](storage-databases.md#blocks-db)
* [Index DB](storage-databases.md#index-db)
* [Use Cases](storage-databases.md#use-cases)

## [Blockchain Security](blockchain-security.md)

* [Transactions](blockchain-security.md#transactions)
* [Transaction Identifiers](blockchain-security.md#transaction-identifiers)
* [Block Identifiers](blockchain-security.md#block-identifiers)
* [Proof of Work and Difficulty](blockchain-security.md#proof-of-work-and-difficulty)

## [RPC Communication](rpc-communication.md)

* [Overview](rpc-communication.md#overview)
* [RPC Flow](rpc-communication.md#rpc-flow)
  * [Initializing](rpc-communication.md#initializing)
  * [Processing Messages](rpc-communication.md#processing-messages)

## [Glossary](glossary.md)

