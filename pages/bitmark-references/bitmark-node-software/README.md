
# Bitmark Node Software


## [Bitmark Blockchain](bitmark-blockchain-technical-overview.md#basic-bitmark-blockchain-overview)

* [Bitmark Blockchain Overview](bitmark-blockchain-technical-overview.md#basic-bitmark-blockchain-overview)
* [Definitions](bitmark-blockchain-technical-overview.md#definitions)
* [Transactions: Asset, Issue and Transfer Records](bitmark-blockchain-technical-overview.md#transactions-asset-issue-and-transfer-records)
    * [Asset Record](bitmark-blockchain-technical-overview.md#asset-record)
    * [Issue Record](bitmark-blockchain-technical-overview.md#issue-record)
    * [Transfer Record](bitmark-blockchain-technical-overview.md#transfer-record)
  * [Transactions: Share]bitmark-blockchain-technical-overview.md#transactions-share)
    * [Bitmark Share]bitmark-blockchain-technical-overview.md#bitmark-share)
    * [Share Grant](bitmark-blockchain-technical-overview.md#share-grant)
    * [Share Swap](bitmark-blockchain-technical-overview.md#share-swap)
  * [Transactions: Block ownership record](bitmark-blockchain-technical-overview.mdd#transactions-block-ownership-record)
    * [Block Foundation](bitmark-blockchain-technical-overview.mdd#block-foundation)
    * [Block Owner Transfer](bitmark-blockchain-technical-overview.md#block-owner-transfer)
  * [Blockchain structure](bitmark-blockchain-technical-overview.md#blockchain-structure)
    * [Block Header](bitmark-blockchain-technical-overview.md#block-header)
    * [Merkle tree](bitmark-blockchain-technical-overview.md#merkle-tree)
    * [Block Hashing (argon2, difficulty, proof of work \.\.\.)](bitmark-blockchain-technical-overview.md#block-hashing-argon2-difficulty-proof-of-work-)


## [Block Verification and Synchronization](block-verification-and-synchronization.md#block-verification-and-synchronization)

* [Bitmark Block](block-verification-and-synchronization.md#bitmark-block)
* [Different types of transactions](block-verification-and-synchronization.md#different-types-of-transactions)
* [Block Validation](block-verification-and-synchronization.md#block-validation)
* [Block Synchronization](block-verification-and-synchronization.md#block-synchronization)
  * [Proof of Work](block-verification-and-synchronization.md#proof-of-work)
  * [Majority Votes](block-verification-and-synchronization.md#majority-votes)

## [Storage](node-modules.md#storage)


## [Bitmark Mining](mining.md##bitmark-mining)

* [Overview](mining.md#overview)
* [Block Diagram](mining.md#block-diagram)
* [Hashing](mining.md#hashing)
* [Difficulty](mining.md#difficulty)
* [Communication](mining.md#communication)
* [Block Verification and Broadcast](mining.md#block-verification-and-broadcast)
* [Reward](mining.md#reward)


##  [Payment Verification](payment-verification.md#payment-verification)

* [Flow](payment-verification.md#flow)
* [Reservoir Module](payment-verification.md#reservoir-module)
  * [Processing a new transaction](payment-verification.md#processing-a-new-transaction)
  * [Generate a payment id](payment-verification.md#generate-a-payment-id)
  * [Change the state of a paid transaction](payment-verification.md#change-the-state-of-a-paid-transaction)
  * [Structures](payment-verification.md#structures)
* [Payment Module](payment-verification.md#payment-module)
  * [Payment Validation](payment-verification.md#payment-validation)
  * [Directly RPC](payment-verification.md#directly-rpc)
  * [Discovery Service](payment-verification.md#discovery-service)
  * [Bitcoin peer\-to\-peer protocol (WIP)](payment-verification.md#bitcoin-peer-to-peer-protocol-wip)


## [Security feature of the Bitmark Blockchain](security.md#security-feature-of-the-bitmark-blockchain)

* [Transactions](security.md#transactions)
* [Transaction Identifiers](security.md#transaction-identifiers)
* [Block Identifiers](security.md#block-identifiers)
* [Proof of Work and Difficulty](security.md#proof-of-work-and-difficulty)

