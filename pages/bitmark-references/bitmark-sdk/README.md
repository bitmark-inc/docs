# Bitmark SDK

## [Overview](bitmark-sdk-document.md#overview)

## [Getting Started](bitmark-sdk-document.md#getting-started)

* [Installing SDK Packages](bitmark-sdk-document.md#installing-sdk-packages)
* [Acquiring an API token](bitmark-sdk-document.md#acquiring-an-api-token)
* [Configuring the SDK](bitmark-sdk-document.md#configuring-the-sdk)


## [Account](account.md#account)

* [Creating an account](account.md#creating-an-account)
* [Retrieving the account number](account.md#retrieving-the-account-number)
* [Exporting an account](account.md#exporting-an-account)
  * [Exporting a seed](account.md#exporting-a-seed)
  * [Exporting a recovery phrase](account.md#exporting-a-recovery-phrase)
* [Importing an account](account.md#importing-an-account)
  * [Recovering from a seed](account.md#recovering-from-a-seed)
  * [Recovering from a phrase](account.md#recovering-from-a-phrase)
* [Signing and verifying](account.md#signing-and-verifying)
* [Validating an account number](account.md#validating-an-account-number)


## [Actions](action.md)

* [Registering an asset](action.md#registering-an-asset)
* [Issuing a bitmark](action.md#issuing-a-bitmark)
* [Transferring a bitmark](action.md#transferring-a-bitmark)
  * [Direct transfer](action.md#direct-transfer)
  * [Countersigned transfer](action.md#countersigned-transfer)
    * [Proposing a transfer offer](action.md#proposing-a-transfer-offer)
    * [Querying for offers](action.md#querying-for-offers)
    * [Accepting a transfer offer](action.md#accepting-a-transfer-offer)
    * [Rejecting a transfer offer](action.md#rejecting-a-transfer-offer)
    * [Canceling a transfer offer](action.md#canceling-a-transfer-offer)


## [Queries](query.md)

* [Asset](query.md#asset)
  * [Record data structure](query.md#record-data-structure)
  * [Querying for a specific asset](query.md#querying-for-a-specific-asset)
  * [Querying for a set of assets](query.md#querying-for-a-set-of-assets)
    * [Assets registered by the specific registrant](query.md#assets-registered-by-the-specific-registrant)
* [Transaction (tx)](query.md#transaction-tx)
  * [Record data structure](query.md#record-data-structure-1)
  * [Querying for a specific transaction](query.md#querying-for-a-specific-transaction)
  * [Querying for a set of transactions](query.md#querying-for-a-set-of-transactions)
    * [Transactions depicting provenance of a bitmark](query.md#transactions-depicting-the-provenance-of-a-bitmark)
    * [Transactions created by an account](query.md#transactions-created-by-an-account)
* [Bitmark](query.md#bitmark)
  * [Record data structure](query.md#record-data-structure-2)
    * [Offer data structure](query.md#offer-data-structure)
  * [Querying for a specific bitmark](query.md#querying-for-a-specific-bitmark)
  * [Querying for a set of bitmarks](query.md#querying-for-a-set-of-bitmarks)
    * [bitmarks issued by the specific issuer](query.md#bitmarks-issued-by-the-specific-issuer)
    * [bitmarks owned by the specific owner](query.md#bitmarks-owned-by-the-specific-owner)
    * [bitmarks offered from the specific sender](query.md#bitmarks-offered-from-the-specific-sender)
    * [bitmarks offered to the specific receiver](query.md#bitmarks-offered-to-the-specific-receiver)


## [Storing a Seed](store-seed.md)
  
* [Android](store-seed.md#android)
* [iOS](store-seed.md#ios)


## [Web Socket](websocket.md#web-socket)
  
* [Event](websocket.md#event)
* [Java](websocket.md#java)
    * [Connect/Disconnect](websocket.md#connectdisconnect)
    * [Subscribe/Unsubscribe](websocket.md#subscribeunsubscribe)
* [Swift](websocket.md#swift)

